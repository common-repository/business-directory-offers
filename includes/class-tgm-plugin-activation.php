...
add_action( 'tgmpa_register', 'wpbdo_register_required_plugins' );

function wpbdo_register_required_plugins() {

	$plugins = get_option('tsl-wpbdo-plugins');

	$config = array(
		'id'           => 'tsl-wbdpo',             // Unique ID for hashing notices for multiple instances of TGMPA.
	...
	if(is_array($plugins)) {
		if (sizeof($plugins) > 0) tsl_wpbdpo_tgmpa($plugins, $config);
	}
}
...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	 ...
	 */
	function tsl_wpbdpo_tgmpa( $plugins, $config = array() ) {
